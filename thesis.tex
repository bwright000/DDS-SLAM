...
The Gaussian-splatting systems are markedly quicker, SGS-SLAM at $5.0$--$5.8$\,s per frame and SemGauss-SLAM at $5.8$--$6.4$\,s, roughly four times faster than our substrate. That comparison is not like-for-like and we do not tabulate it: SemGauss-SLAM's completed runs were executed on an $80$\,GB card while ours ran on a $15$\,GB T4, and the benchmark harnesses did not record device identity at run time, so a wall-clock column would report the hardware at least as much as the method. One accident bounds that confound. SemGauss-SLAM's C\_1/001 run landed on a T4 rather than the $80$\,GB card and took $14.0$\,s per frame against $5.8$--$6.4$\,s on its other snippets, a device factor of roughly $2.3$. Discounting the hardware on that basis still leaves rasterising a Gaussian map cheaper per frame than ray-sampling a signed-distance field, which is a property of the representation rather than of either implementation.

The ranking matters less than the magnitude. Real-time surgical tracking requires $30$\,fps; DID-SLAM is some $795\times$ short of it and the fastest system benchmarked here is still $156\times$ short, so every method evaluated in this thesis sits two to three orders of magnitude away from the regime in which it could inform a surgeon during an operation. A gap that wide does not close by tuning, and the plausible route to it is hardware rather than implementation. It is worth being specific about which hardware: a datacentre A100 is not deployable beside a patient, whereas the T4-class accelerator our own timings were taken on is nearer the envelope an embedded surgical system could carry. The numbers reported here are therefore the deployment-relevant ones rather than the flattering ones, and the speed-up that would matter is the one that arrives as accelerators become faster and more compact, not as this pipeline becomes better tuned.
