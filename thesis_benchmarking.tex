...
G3\_001 could not be benchmarked: every attempt --- the faithful configuration
and a pruning-tightened variant (densification gate 0.5$\rightarrow$0.25, prune
threshold 0.005$\rightarrow$0.05) --- exhausted an A100-80GB partway through the
sequence and never completed. The failure is a \emph{scaling} property of the
representation rather than a transient crash. SplaTAM-family methods add
Gaussians wherever the current map fails to explain the frame, so memory grows
with newly-revealed surface rather than with sequence length, and G3\_001's
exploratory camera and tool activity never let densification plateau --- it had
accumulated 11.0\,M Gaussians, six times C3\_001's final count (1.75--1.78\,M
over a longer 1527-frame sequence) at a fraction of the frames, when it crossed
the 80\,GB ceiling. SGS-SLAM completed the same snippet on the same GPU because
its Gaussians carry no 16-dimensional feature vector, no 16-channel
rasterisation buffers and no corresponding gradients, and are roughly
$3\times$ lighter. The authors' configuration exposes no Gaussian budget ---
Replica never needed one --- so the ceiling is a property of the method, not of
our port: the semantic feature field trades memory scalability for its dense
semantics, and an exploratory endoscopic sequence is precisely the regime that
collects the bill. G3\_001 is therefore reported as a did-not-finish; the four
completed snippets carry the SemGauss-SLAM comparison.
