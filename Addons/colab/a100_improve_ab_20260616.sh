...
[ "${ALLOW_RECTIFIED:-0}" = 1 ] || { echo "🚨 a100_improve_ab is DEPRECATED (rectifies CRCD). RAW-LEFT replacement: dino_ab_rawleft_isolated_20260618.sh  (CELLS=\"base geo\" SEEDS=\"0 1 2\" = the IMPROVE A/B). ALLOW_RECTIFIED=1 to force a deliberate rectified run." >&2; exit 1; }
DATE=$(date +%Y%m%d)
...
for cond in ['c1_001_canon_base','c1_001_canon_uncert']:
    ...
    A=np.array(P)
    print(f"  {cond:<24} n={len(P)}  PSNR={A[:,0].mean():5.2f}+/-{A[:,0].std():.2f}  "
          f"SSIM={A[:,1].mean():.3f}  LPIPS={A[:,2].mean():.3f}  (frames~{int(A[:,3].mean())})")
