...
say "READ-OUT: per test validate.json -> live_verdict (DEAD / DIVERGED / STATIC-GAUGE / LIVE)."
say "  LIVE = finite mean_norm (NOT 0, NOT >1e3) + temporal_frac>0.05. That reg = the stable point."
say "  Then re-run oracle A/B (pb_control vs pb_oracle) at that reg -> answers T1.3."
say "  If NONE is LIVE (all DEAD or DIVERGED): band is narrower/absent -> try lr_mult 0.01, or pivot map-side."
