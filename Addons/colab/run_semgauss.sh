...
semgauss_write_cfg(){
  ...
  # NON-FAITHFUL ablation arm (SEMGAUSS_VARIANT=tightprune): tightens ONLY the densify/prune knobs to
  # cut Gaussian count (~2-4x) so exploratory scenes (G3: 11M gaussians @ frame 600 vs C3 1.8M full-run)
  # fit full-length in 80GB. Labelled + segregated everywhere; NEVER the main-table row (the faithful
  # NUM_FRAMES-capped run is). Default unset -> byte-verbatim authors' values.
  local VSUF="" MAP_SIL_ADD=0.5 PR_RMOP=0.005 PR_EVERY=20 PR_STOP=20
  if [ "${SEMGAUSS_VARIANT:-}" = tightprune ]; then
    VSUF="_tightprune"; MAP_SIL_ADD=0.25; PR_RMOP=0.05; PR_EVERY=10; PR_STOP=60
    echo "[cfg] ⚠ VARIANT tightprune: sil_thres(add) 0.5->0.25, prune rm_op 0.005->0.05, every 20->10, stop 20->60"
  elif [ -n "${SEMGAUSS_VARIANT:-}" ]; then
    echo "[cfg] FATAL unknown SEMGAUSS_VARIANT='$SEMGAUSS_VARIANT' (supported: tightprune)"; return 1
  fi
  mkdir -p "$(dirname "$cfg")"
  cat > "$cfg" <<PYCFG
...
group_name = "CRCD"
run_name = f"{scene_name}${VSUF}_{seed}"   # VSUF empty on faithful runs -> authors' naming
...
config = dict(
    ...
    mapping=dict(
        ...
        pruning_dict=dict(
            ...
            remove_big_after=0,
            stop_after=$PR_STOP,
            prune_every=$PR_EVERY,
            removal_opacity_threshold=$PR_RMOP,
...
run_crcd_one(){
  ...
  # variant segregation: ablation arms get their OWN Drive dir/run-name -> can never overwrite or
  # masquerade as the faithful row.
  local VS=""; [ -n "${SEMGAUSS_VARIANT:-}" ] && VS="_${SEMGAUSS_VARIANT}" \
    && echo "[$UP] ⚠ NON-FAITHFUL ablation arm '$SEMGAUSS_VARIANT' -> outputs to ${UP}${VS} (side-note only, never the main table)"
...
PY
  ...
  local cfg="$SEMGAUSS/configs/crcd/bench_${UP}${VS}.py"
  ...
  ( cd "$SEMGAUSS" && PYTORCH_CUDA_ALLOC_CONF="${PYTORCH_CUDA_ALLOC_CONF:-max_split_size_mb:128}" \
      "$ENV_PY" sem_gauss.py "configs/crcd/bench_${UP}${VS}.py" ) 2>&1 | tee "$OUT/run.log"
  ...
  # exact run dir ${UP}${VS}_${SEED}: a bare ${UP}_* glob would let a FAITHFUL re-eval pick up a
  # tightprune checkpoint (G3_001_* matches G3_001_tightprune_2027) -> cross-arm contamination.
  local NPZ; NPZ=$(ls -t "$SEMGAUSS"/experiments/*/${UP}${VS}_${SEED}/params.npz 2>/dev/null | head -1)
  [ -n "$NPZ" ] || NPZ=$(ls -t "$SEMGAUSS"/experiments/*/${UP}${VS}_${SEED}/params*.npz 2>/dev/null | head -1)
  [ -n "$NPZ" ] && [ -f "$NPZ" ] || { echo "FAILED no params.npz (experiments/*/${UP}${VS}_${SEED})" > "$OUT/status.txt"; touch "$OUT/.FAILED"; return 1; }
...
PY
  ...
  PYTHONPATH= "$DDS_PY" - "$OUT" "$UP$VS" <<'PY'
