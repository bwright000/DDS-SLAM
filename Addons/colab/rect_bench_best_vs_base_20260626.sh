...
if echo " $ARMS " | grep -qE " (calm_v3gate|loo_unc|loo_calm) "; then
  python -c "import transformers" 2>/dev/null || { echo "[env] installing transformers for the v3gate arm"; pip install -q -U transformers; }
fi
if echo " $ARMS " | grep -qE " (best|vote|vote_trust|vote_both|dtrust|uncdtrust|dpool|uncdpool|uncvote_fba|uncvote_fba_baw|uncvotetrust_fba|uncvote_fba_calm|uncvote_fba_calm_r30|uncvote_fba_calm_r35|calm_d42|calm_d33|calm_prior1|calm_prior10|calm_v3gate|loo_unc|loo_calm) "; then
