...
declare -A ARM_TMPL=( [base]=configs/CRCD/crcd_improved_rect.yaml [best]=${BEST_CFG:-configs/CRCD/crcd_best_rect.yaml} \
  ...
  [oracle_fba]=configs/CRCD/crcd_abl_oracle_fba_rect.yaml [vote_trust]=configs/CRCD/crcd_abl_vote_trust_rect.yaml \
  [vote_both]=configs/CRCD/crcd_abl_vote_both_rect.yaml [dtrust]=configs/CRCD/crcd_abl_dtrust_rect.yaml \
  [uncdtrust]=configs/CRCD/crcd_abl_uncdtrust_rect.yaml [uncdpool]=configs/CRCD/crcd_abl_uncdpool_rect.yaml \
  ...
  [calm_v3gate]=configs/CRCD/crcd_abl_calm_v3gate_rect.yaml   [loo_unc]=configs/CRCD/crcd_abl_loo_unc_rect.yaml [loo_gate]=configs/CRCD/crcd_abl_loo_gate_rect.yaml   [loo_calm]=configs/CRCD/crcd_abl_loo_calm_rect.yaml   [dinobudget_a30]=configs/CRCD/crcd_abl_dinobudget_a30_rect.yaml [dinobudget_a50]=configs/CRCD/crcd_abl_dinobudget_a50_rect.yaml   [dinobudget_a70]=configs/CRCD/crcd_abl_dinobudget_a70_rect.yaml )   # BEST_CFG= override (T4). Arms: abl_*/l0*/dpool/pnp | prior (DDS_MP_LAM sweep) | cons=epoch-consistent poses | unc/uncfix=canon-confound A/B | still/stillcons=map gate (v1 FAILED) | vote=C' freeze | vote_trust=vote as DOWN-WEIGHT (no freeze) | vote_both=freeze+trust | oracle=GT-perfect freeze (ceiling) | *_fba=+freeze_ba
...
if echo " $ARMS " | grep -qE " (calm_v3gate|loo_unc|loo_calm|dinobudget_a30|dinobudget_a50|dinobudget_a70) "; then
  python -c "import transformers" 2>/dev/null || { echo "[env] installing transformers for the v3gate arm"; pip install -q -U transformers; }
fi
if echo " $ARMS " | grep -qE " (best|vote|vote_trust|vote_both|dtrust|uncdtrust|dpool|uncdpool|uncvote_fba|uncvote_fba_baw|uncvotetrust_fba|uncvote_fba_calm|uncvote_fba_calm_r30|uncvote_fba_calm_r35|calm_d42|calm_d33|calm_prior1|calm_prior10|calm_v3gate|loo_unc|loo_calm|dinobudget_a30|dinobudget_a50|dinobudget_a70) "; then
