...
def probe_frame(slam, dataset, frame_id, trans_perts_m, rot_perts_deg):
    ...
    results = {'frame_id': frame_id, 'baseline': baseline, 'trans': {}, 'rot': {}}
    axis_names = ['x', 'y', 'z']
...
def main():
    ...
    for res in all_results:
        ...
        for i, aname in enumerate(['x', 'y', 'z']):
            ...
            ax[0, i].plot(dts, losses, '-o', lw=1, ms=3)
            ax[0, i].axvline(0, color='k', lw=0.5, alpha=0.3)
            ax[0, i].axhline(res['baseline']['total'], color='r', lw=0.5, alpha=0.3, label='baseline')
            ax[0, i].set_title(f'trans_{aname} (frame {fid})')
            ...
            ax[1, i].plot(dts_r, losses_r, '-o', lw=1, ms=3)
            ax[1, i].axvline(0, color='k', lw=0.5, alpha=0.3)
            ax[1, i].axhline(res['baseline']['total'], color='r', lw=0.5, alpha=0.3)
            ax[1, i].set_title(f'rot_{aname} (frame {fid})')
