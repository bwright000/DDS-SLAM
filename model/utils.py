...
def compute_loss(prediction, target, border=None, loss_type='l2', weights=None, UsePercentage=False, percentage=0.8):
    ...
    Params: 
        ...
        loss_type: str
        weights: torch.Tensor, (Bs, N_samples)
        percentage: float, percentage of values to consider for loss calculation
    Return:
        loss: torch.Tensor, (1,)
    '''
    if weights is None:
    ...
    if UsePercentage is False:
        ...
        if border is None:
            border = torch.zeros_like(target)
        exclude_mask = (border == 1)
        abs_diff[exclude_mask] = 0
        
        num_values = abs_diff.numel()
        num_values_to_keep = int(percentage * num_values)
        flat_abs_diff = abs_diff.view(-1)
        sorted_indices = torch.argsort(flat_abs_diff)
        threshold_value = flat_abs_diff[sorted_indices[num_values_to_keep - 1]]
        if threshold_value < 0.2:
            threshold_value = 0.2
            mask = 1 - (abs_diff >= threshold_value).float()
            loss = F.mse_loss(prediction*mask, target*mask)
            return loss
        loss_mask = torch.zeros_like(flat_abs_diff)
        loss_mask[sorted_indices[:num_values_to_keep]] = 1
        loss = F.mse_loss(prediction * loss_mask.view_as(prediction), target * loss_mask.view_as(target))
        return loss
